...
$(document).ready(() => {
  ...
  const toggle = localStorage.getItem('currentToggle');
  ...
  buttonClickCallbackNightTheme = (e) => {
    e.preventDefault();
    setTimeout(() => {
      const recentContribsSectionImgs = document.getElementsByClassName('recent-contributors')[0].getElementsByTagName('img');
      const check = localStorage.getItem('currentToggle');
