...
document.addEventListener('DOMContentLoaded', function () {
  ...
  (function () {
    ...
    if (repo === 'all') {
      toolbox
        .getIssuesForOrg(org, { qs: { labels: ftoLabel } })
        .then(displayIssuesAndFtoAuthors('.first-timers-only'));

      toolbox
        .getIssuesForOrg(org, { qs: { labels: candidateLabel } })
      ...
      toolbox.api.Issues.getIssuesForRepo(org, repo, {
        qs: { labels: ftoLabel }
      }).then(displayIssuesAndFtoAuthors('.first-timers-only'));

      toolbox.api.Issues.getIssuesForRepo(org, repo, {
        qs: { labels: candidateLabel }
      }).then(displayIssuesAndFtoAuthors('.candidates'));
      ...
      return function displayIssuesAndFtoAuthors (issues) {
        ...
        issues.forEach(function (issue) {
          ...
          issue.labels.forEach(ftoLabel => {
            if (
              ftoLabel.name === 'first-timers-only' ||
              ftoLabel.name === 'fto'
            ) {
