...
const getContribsLastWeek = require('../../src/utils/recentContribsUtil/getContribsLastWeek');
const storeAllRecentContribsInDb = require('../../src/utils/recentContribsUtil/storeAllRecentContribsInDb');
...
describe('main.js', () => {
  ...
  beforeAll(() => {
    ...
    getContribsLastWeek.getContribsLastWeek = jest.fn(() =>
      Promise.resolve(contribs)
    );
    storeAllRecentContribsInDb.storeAllRecentContribsInDb = jest.fn(() =>
      Promise.resolve(contribs)
    );
  });
  ...
  it('fetches contributors for desired months by calling getContribsLastMonth', () => {
    return fetchContribsLastMonth(org, repo).then(data => {
  ...
  it('fetches contributors for last week by calling getContribsLastWeek', () => {
    return fetchContribsLastWeek(org, repo).then(data => {
      expect(data).toBeDefined();
      expect(data.length).toBeGreaterThan(0);
      expect(data[0].author.login).toBeDefined();
    });
  ...
  it('fetches and stores all recent contributors by calling fetchAllRecentMonthContribs', () => {
    return fetchAllRecentContribsInDb(org, repo).then(data => {
      expect(data).toBeDefined();
      expect(data.length).toBeGreaterThan(0);
      expect(data[0].author.login).toBeDefined();
    });
