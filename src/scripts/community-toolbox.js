CommunityToolbox = function CommunityToolbox(org, repo) {
        ...
        .then(dummy => {
          return model_utils.getItem("repos").then(response => {
            if (response == null || response == undefined) {
              ...
              return fetchReposUtil.getAllRepos(org).then(resp => {
                resolve(true);
              });
            }
            resolve(true);
  ...
  function showAllContributors(org) {
    return contributorsUtil
      .storeAllContributorsInDatabase(org)
  ...
  function showRecentContributors(org, repo, recencyLabel) {
    return contributorsUtil
      .storeAllRecentContribsInitially(org, repo)
      .then(result => {
        if (recencyLabel === "month") {
          return recentContribsUtil
            .getCommitsLastMonth(org, repo)
